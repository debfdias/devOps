...
export const createPost = ({ title}) => {
  ...
  return (dispatch) => {
    ...
    firebase.database().ref(`/users/${currentUser.uid}/post`)
      ...
      .catch(() => {
        dispatch({ type: POST_CREATE_FAILURE, payload: 'Registrado com sucesso' });
      });
  };
};

export const updatePost = ({ title, uid }) => {
  const { currentUser } = firebase.auth();
  ...
  return (dispatch) => {
    ...
    firebase.database().ref(`/users/${currentUser.uid}/post/${uid}`)
      ...
      .catch(() => {
        dispatch({ type: POST_UPDATE_FAILURE, payload: 'Falha ao salvar' });
      });
  };
};
...
export const deletePost = ({ uid }) => {
  ...
  return (dispatch) => {
    ...
    firebase.database().ref(`/users/${currentUser.uid}/post/${uid}`)
      ...
      .catch(() => {
        dispatch({ type: POST_DELETE_FAILURE, payload: 'Falha ao deletar' });
      });
  };
};
